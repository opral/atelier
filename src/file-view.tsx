...
export type AtelierFileViewProps = {
	readonly lix: Lix;
...
export function FileView(props: AtelierFileViewProps) {
	if (props.diff && "baseCommitId" in props.diff && !props.targetCommitId) {
		throw new Error(
			"Atelier.FileView requires an explicit target commit for a diff.",
		);
	}
	if (!props.fileId) throw new Error("Atelier.FileView requires a fileId.");
...
function FileViewContent(props: AtelierFileViewProps) {
...
function unavailable(): never {
	throw new Error("This action requires Atelier.Shell.");
}

function MountedFile(
	props: AtelierFileViewProps & {
		path: string;
		branchId: string;
