...
describe("undo after an outside write", () => {
	const DOC =
		"# Title\n\nAlpha one.\n\nBravo two.\n\nCharlie three.\n\nDelta four.\n";

	test("undoing a split keeps the block the agent put between its halves", () => {
		const editor = editorFor(DOC);
		editor.commands.setTextSelection(at(editor, 2, "Bravo".length));
		editor.commands.splitBlock();
		apply(
			editor,
			saved(editor).replace("Bravo\n\n", "Bravo\n\nAgent block.\n\n"),
		);
		expect(texts(editor)).toContain("Agent block.");
		editor.commands.undo();
		expect(texts(editor)).toEqual([
			"Title",
			"Alpha one.",
		...
		// Redo splits again, and the agent's block stays.
		editor.commands.redo();
		expect(texts(editor)).toContain("Agent block.");
		expect(texts(editor)).toContain("Bravo");
		expect(saved(editor)).toContain("Agent block.");
	});

	test("undoing typing keeps the agent's text written inside it", () => {
		const editor = editorFor(DOC);
		editor.commands.setTextSelection(at(editor, 1, "end"));
		editor.commands.insertContent(" typed");
		apply(editor, saved(editor).replace(" typed", " ty(agent)ped"));
		editor.commands.undo();
		expect(texts(editor)[1]).toContain("(agent)");
	});
});
