...
describe("a view that does not render its own diff", () => {
	test("is compared with itself, before beside after", async () => {
		const lix = await openLix();
		const objectUrls = vi.fn((_blob: Blob) => "blob:atelier-image");
		Object.defineProperty(URL, "createObjectURL", {
			configurable: true,
			value: objectUrls,
		});
		...
		try {
			...
			await qb(lix)
				.updateTable("lix_file")
				.where("id", "=", fakeUuid("compare-shot"))
			...
			// The image view has no diff of its own, so the shell mounts it twice
			...
			const before = utils!.container.querySelector<HTMLElement>(
				"[data-diff-side='before']",
			)!;
			const after = utils!.container.querySelector<HTMLElement>(
				"[data-diff-side='after']",
			)!;
			expect(before).toHaveAccessibleName("Before: shot.png");
			expect(after).toHaveAccessibleName("After: shot.png");
			await waitFor(() => {
				expect(
					before.querySelector("[data-testid='image-viewer']"),
				).not.toBeNull();
				expect(
					after.querySelector("[data-testid='image-viewer']"),
				).not.toBeNull();
			});
			// The review stays in charge: neither pane opens a session of its own.
			expect(
				await screen.findByRole("button", { name: /^Checkpoint(ing…)?$/ }),
			).toBeVisible();
		} finally {
